...
Первинні джерела прочитано без реальних облікових даних. Доповнення від 24.09.2026 з обмеженою реальною read-only перевіркою Prom: [FISCAL_LINK_REPORT.md](FISCAL_LINK_REPORT.md). Нижче розділено підтверджений API-контракт і його обмеження.
...
Уточнення 24.09.2026: у реальному локальному кеші Prom `price` містить явний суфікс `грн`. Виправлений parser підтримує його та інші явні UAH-позначення, кому/крапку для копійок і коректно згруповані пробіли тисяч; UAH визначається із суфікса, а не припускається з назви маркетплейсу. Без суфікса валюта лишається невідомою. Подробиці та межі — у [PROM_CONTRACT_NOTES.md](PROM_CONTRACT_NOTES.md).
...
З 24.09.2026 точні ключі моделюються окремо в `FiscalReferences`: UUID, перевірений Checkbox URL, фіскальний код із контекстом. `ReceiptIds=[]` реальних адаптерів більше не обмежує matcher: Rozetka додає типізовані ключі з PRRO-полів і read-only URL. Для підтвердженого фіскального номера використовується точне порівняння `ReceiptRecord.FiscalCode`, не `Id` чи `Serial`. Потрібен відомий контекст каси або організації з однозначною read-only перевіркою в Checkbox. Суми/дати формують тільки кандидатів. Детальні умови та межі перевірки — у [FISCAL_LINK_REPORT.md](FISCAL_LINK_REPORT.md).

Невідомі URL й фіскальні номери без потрібного контексту залишаються довідковими, а не доказом. Формат seller-order URL не підтверджений офіційним контрактом обох сервісів: посилання кабінету не вигадуються. До allowlist додано лише документований GET `/prro/receipt-status/{order_id}`; він потрібен, коли статус відсутній у detail-відповіді. `DetailExpand` не розширюється недокументованим `prro`.
