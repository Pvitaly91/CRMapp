...
Доповнення 24.09.2026: UUID-подібний рядок у фіскальному номері або коментарі не стає точним ключем. Але строго перевірений Checkbox HTML URL тепер перетворюється на окремий типізований ключ `FiscalReferences`, а не залишається лише display-only. Підтверджений UUID/URL або code з достатнім контекстом може створити автоматичний зв’язок. Відсутнє `prro` у деталях перевіряється документованим GET статусу, list-only поля й старі ключі не губляться. Докладні правила, synthetic тести й обмеження без live Rozetka — [FISCAL_LINK_REPORT.md](FISCAL_LINK_REPORT.md). Формат URL сторінки замовлення в кабінеті продавця не вгадується.
