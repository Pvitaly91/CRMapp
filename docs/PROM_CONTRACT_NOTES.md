...
Actual authenticated read access, optional fields returned by a particular store, response currency format and cursor behavior still require local testing with the user's token. Physical printing is not part of the Prom adapter verification.
